...
> **⏸ 부분 처리 (PR-2a) + 보류 (메뉴 IA 재배치 PR 과 함께).**
> 메뉴 항목 정리도 같이 가야 그룹명-내용 mismatch 안 생김.
> PR-2a 에서 부분 처리: '운영 → 홈' (그룹 내용이 이미 일관) 만 즉시 변경.
> '관리 → 제품 운영' 변경은 보류 — 현재 '관리' 그룹 안에 제품 운영성
> 메뉴(계정/아바타)와 인프라성 메뉴(워커/감사 로그/설정)가 섞여있어,
> 라벨만 바꾸면 새 mismatch 발생. 메뉴 IA 재배치(PR-3 이후)와 함께
> '관리' 분할 + 라벨링 동시 진행.
