...
def perform_update(repo_dir: str | None = None) -> None:
    ...
    Slice 1 — HYDRA_UPDATE_OWNER 게이트.
    기본값 "self" 는 현행 desktop worker 자가 update 유지.
    Phase 3 에서 Admin Agent 가 update 를 소유하면 "agent" 로 설정 — 그 경우
    self-update 거부 (Agent 가 stop→git→pip→start 흐름을 owned).
    """
    if repo_dir is None:
        repo_dir = _default_repo_dir()

    owner = os.environ.get("HYDRA_UPDATE_OWNER", "self")
    if owner != "self":
        log.warning(
            "updater: self-update disabled (HYDRA_UPDATE_OWNER=%s, expected admin agent owned)",
            owner,
        )
        raise RuntimeError(
            f"self-update disabled; HYDRA_UPDATE_OWNER={owner} — update is owned by admin agent"
        )
