...
def _count_available_accounts(db: Session, exclude_ids: set[int] | None = None) -> int:
    """현재 가용 active 계정 수."""
...
def create_campaign_with_slot_tasks(
    ...
    *,
    ...
    for s in slots:
        ...
        # 가용 계정 수 캡: 영상당 1계정 1좋아요 원칙 + 댓글 슬롯에 배정된 계정 제외
        # (재등장은 같은 계정이라 1번만 카운트 → set 자동 dedupe)
        if like_n > 0:
            slot_account_ids = {a.id for a in label_to_account.values()}
            available_for_likes = _count_available_accounts(
                db, exclude_ids=slot_account_ids
            )
            if available_for_likes < like_n:
                log.warning(
                    "slot %s: like_n=%d capped to %d (active accounts shortage; "
                    "campaign=%s video=%s)",
                    s.slot_label, like_n, available_for_likes,
                    campaign.id, video_id,
                )
                like_n = available_for_likes
        if like_n > 0:
