...
async def async_human_scroll_into_view(
    ...
    # hydra patch (mirror of sync scroll.py): viewport-verify, retry once,
    # explicit raise if still off-screen so caller can fallback / capture UNKNOWN.
    if not _is_in_viewport(box, viewport_height, cfg):
        await async_sleep_ms(rand_range(cfg.scroll_settle_delay))
        box = await get_box()
        if box is None or not _is_in_viewport(box, viewport_height, cfg):
            raise RuntimeError(
                f"Element not in viewport after async scroll (y={box['y'] if box else 'None'}, "
                f"viewport_h={viewport_height}) — caller should fallback or capture UNKNOWN_SCREEN"
            )
