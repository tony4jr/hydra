...
from .config import rand, rand_range, rand_int_range, sleep_ms, async_sleep_ms
from .mouse import RawMouse, Point, human_move, human_click, click_target, human_idle
from .keyboard import RawKeyboard, human_type, SHIFT_SYMBOLS, NEARBY_KEYS
...
# hydra namespace logger (was cloakbrowser.human in upstream)
logger = logging.getLogger("hydra.browser.human")
...
def _patch_locator_class_sync():
    ...
    def _humanized_select_option(self, value=None, **kwargs):
        if _is_humanized(self):
            ...
            return _orig_select_option(self, value, **kwargs)
        return _orig_select_option(self, value, **kwargs)
...
def _patch_locator_class_async():
    ...
    async def _humanized_select_option(self, value=None, **kwargs):
        if _is_humanized(self):
            ...
            return await _orig_select_option(self, value, **kwargs)
        return await _orig_select_option(self, value, **kwargs)
...
def _patch_single_element_handle_sync(
    ...
    # --- el.press() ---
    # hydra patch (Codex P2): Playwright ElementHandle.press() focuses element
    # first; upstream sent keys to currently-focused element (wrong target/drop).
...
def _patch_single_frame_sync(
    ...
    # hydra patch (Codex P2): route via frame.locator so iframe scope is preserved.
    # Upstream forwarded to page.click(selector) which resolves in main-frame
    # context → iframe-only elements timeout or hit wrong-frame siblings.
    # Locator class is already humanized, so the action stays human-like.
    def _frame_click(selector: str, **kwargs: Any) -> None:
        frame.locator(selector).first.click(**kwargs)

    def _frame_dblclick(selector: str, **kwargs: Any) -> None:
        frame.locator(selector).first.dblclick(**kwargs)

    def _frame_hover(selector: str, **kwargs: Any) -> None:
        frame.locator(selector).first.hover(**kwargs)

    def _frame_type(selector: str, text: str, **kwargs: Any) -> None:
        frame.locator(selector).first.type(text, **kwargs)

    def _frame_fill(selector: str, value: str, **kwargs: Any) -> None:
        frame.locator(selector).first.fill(value, **kwargs)

    def _frame_check(selector: str, **kwargs: Any) -> None:
        frame.locator(selector).first.check(**kwargs)

    def _frame_uncheck(selector: str, **kwargs: Any) -> None:
        frame.locator(selector).first.uncheck(**kwargs)
...
def _patch_single_element_handle_async(
    ...
    # --- el.press() ---
    # hydra patch (Codex P2 mirror): focus element before sending keys.
