...
def human_scroll_into_view(
    ...
    # hydra patch: 휠 이벤트로 viewport 진입 실패한 경우 — scroll 경계, 중첩 스크롤,
    # lazy-load 미작동 등. 호출자가 box로 click 시도하면 화면 밖 클릭됨.
    # 한 번 더 settle 후 재확인, 여전히 밖이면 명시적으로 실패 raise.
    if not _is_in_viewport(box, viewport_height, cfg):
        sleep_ms(rand_range(cfg.scroll_settle_delay))
        box = get_box()
        if box is None or not _is_in_viewport(box, viewport_height, cfg):
            raise RuntimeError(
                f"Element not in viewport after scroll (y={box['y'] if box else 'None'}, "
