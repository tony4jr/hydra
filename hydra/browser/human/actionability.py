...
def ensure_actionable_handle(
    ...
    while True:
        ...
        try:
            # hydra patch: explicit attached check for handle (Codex P2 — upstream
            # cloakbrowser returns success without verifying when only 'attached' is
            # in checks). owner_frame()/evaluate fail on detached handles.
            if "attached" in checks:
                try:
                    el.evaluate("e => e.isConnected")
                except Exception:
                    raise ElementNotAttachedError(label)
